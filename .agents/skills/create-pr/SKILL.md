...
4. **Create or update PR**
   ...
   ```bash
   gh pr list --state open --head "$branch" --json number,url,title,body,baseRefName,headRefName --limit 1
   ```
   If that command returns an open PR, update it instead of creating a duplicate:
   ```bash
