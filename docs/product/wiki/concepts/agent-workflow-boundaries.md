...
- Implementation agent 负责读取 workflow 提供的稳定上下文路径、产出实现 diff、必要时同步 specs，并把 `implementation_summary.md` 与 `pr-metadata.json` 写到 workflow 指定的 `.codex-runtime/handoff/` 目录。
