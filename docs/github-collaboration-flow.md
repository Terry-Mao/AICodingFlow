...
1. 在 runner 临时 handoff 目录准备 `issue_context.json` 和 `issue_comments.txt`，并把具体路径传给 Codex 和后续脚本。
2. Codex 按顺序使用 `spec-driven-implementation`、`write-product-spec`、`create-product-spec`、`write-tech-spec`、`create-tech-spec`。
3. 生成 `specs/issue-<N>/product.md`、`specs/issue-<N>/tech.md`，并把 `pr-metadata.json` 写到同一个临时 handoff 目录。
...
1. 在 runner 临时 handoff 目录准备 `issue_context.json`、`issue_comments.txt`，有 spec context 时生成 `spec_context.md`。
...
5. Codex 留下实现 diff，并把 `implementation_summary.md` 和 `pr-metadata.json` 写到临时 handoff 目录。
...
1. `prepare_pr_comment_context.py` 在 runner 临时 handoff 目录生成 `pr_comment_context.json`、`pr_event.json` 和 `review_comment_ids.json`。
2. workflow checkout PR head，并在同一个临时 handoff 目录生成 `pr_diff.txt` 和可选 `spec_context.md`。
3. Codex 使用 `implement-specs`、`spec-driven-implementation`、`implement-issue`，按触发 comment 的范围做最小修复。
4. Codex 把 `implementation_summary.md`、`pr-metadata.json`，以及必要时的 `resolved_review_comments.json` 写到临时 handoff 目录。
...
这些 implementation、spec 和 `/fix` handoff artifact 由 workflow 放在 runner 临时目录中，artifact 上传仍保留文件名；它们不再依赖仓库根目录 `.gitignore` 来隐藏 scratch 文件。
