...
- `pull_request.labeled` 场景只在 label 为 `plan-approved` 时继续，并从 PR title/body/head ref 解析 issue number，再 fetch issue 确认 readiness。
...
- 新增 `tests/test_prepare_issue_implementation_context.py`：
  ...
  - `pull_request.labeled` 只有 `plan-approved` 才触发，并会校验 issue readiness。
