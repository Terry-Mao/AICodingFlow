...
class ReviewWorkflowDispatchTest(unittest.TestCase):
    ...
    def test_create_implementation_workflow_does_not_dispatch_review_after_pr_creation(self) -> None:
        ...
        self.assertEqual(app_token["with"]["client-id"], "${{ vars.WORKFLOW_UPDATE_APP_CLIENT_ID }}")
        self.assertEqual(app_token["with"]["private-key"], "${{ secrets.WORKFLOW_UPDATE_APP_PRIVATE_KEY }}")
